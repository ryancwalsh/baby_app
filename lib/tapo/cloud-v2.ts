  ...
  return new Promise((resolve, reject) => {
    ...
    client.on("socket", (socket: TLSSocket) => {
      socket.on("secureConnect", () => {
        ...
        if (!isPinned) {
          client.destroy(
            new Error("The TP-Link certificate did not match the pin."),
          );
        }
      });
    });
    ...
    client.on("timeout", () =>
      client.destroy(new Error("Tapo cloud timeout.")),
    );
...
function storeSession(result: CloudResult["result"], terminalUuid: string) {
  if (result?.token === undefined) {
    throw new Error(
      result?.errorMsg ?? "The Tapo cloud did not return a token.",
    );
  }
  saveTokens({
    token: result.token,
 ...
 */
export async function sendToTapoDevice(
  deviceId: string,
  requestData: unknown,
): Promise<unknown> {
  ...
  const body = await post(
    PASSTHROUGH_PATH,
    queryString(tokens.terminalUuid, { token: tokens.token }),
