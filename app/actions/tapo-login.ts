...
export async function getTapoCloudStatusAction(
  secretHash: string,
...
export async function startTapoCloudLoginAction(
  secretHash: string,
): Promise<{ needsMfaCode: boolean }> {
  requireLogin(secretHash);
  return startTapoCloudLogin();
}

export async function submitTapoMfaCodeAction(
  secretHash: string,
  code: string,
): Promise<void> {
  requireLogin(secretHash);
